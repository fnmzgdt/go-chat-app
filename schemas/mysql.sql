CREATE TABLE IF NOT EXISTS users (id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT, username VARCHAR(40) NOT NULL, email VARCHAR(255) NOT NULL, password VARBINARY(60) NOT NULL, profile_image_url VARCHAR(255) NULL);

CREATE TABLE IF NOT EXISTS threads (id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT, name VARCHAR(40) NULL, type TINYINT UNSIGNED NOT NULL, created_at TIMESTAMP NOT NULL, created_by INT UNSIGNED NULL, FOREIGN KEY(created_by) REFERENCES users(id) ON DELETE CASCADE);
